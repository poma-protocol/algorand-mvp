...
export const ReferralClaim1 = ({
  ...
  const initialOptin = async (tokenId: number) => {
    ...
    if (activeAccount) {
      try {
        ...
        openMessage(messageApi, key, "Signature Received, Sending Transaction...");
  ...
  const onClick = async (form: FormInstance) => {
    ...
    const result = await fetch(BACKEND_URL + "/token-transfer-2", {
      method: "POST",
          ...
          rules={[
            {
              type: "method",
              required: true,
              message: "Invalid Referral Count",
              validator: async (_, value) => {
